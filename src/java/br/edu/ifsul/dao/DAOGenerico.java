...
public class DAOGenerico<T> implements Serializable {
    ...
    private Class classPersistente;
    ...
    public List<T> getListaObjeto() {
        ...
        String where = "";
        filtro = filtro.replaceAll("[';-]",""); //protegendo contra sql inject
        if(filtro.length()>0){
            if(ordem.equals("id")){
                try {
        ...
        jpql += where;
        jpql += " order by " + ordem;
        totalObjetos = em.createQuery("select id from " + classPersistente.getSimpleName() + 
    ...
    public boolean remove(T obj) {
        try {
            ...
            em.remove(obj);
            em.getTransaction().commit();
            mensagem = "Objeto remover com sucesso";
            return true;
        } catch (Exception e) {
            if (em.getTransaction().isActive() == false) {
                em.getTransaction().begin();
    ...
    public T localizar(Integer id){
        return (T) em.find(classPersistente, id);
    }
